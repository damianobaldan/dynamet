...
#'
#' @export
dynamicMetacomm_test <- function(nEpochs, ..., init.comm = NULL, updater = NULL) {
  ...
  additional_output <- vector("list", nEpochs)
  ...
  for (i in 1:nEpochs) {
    ...
    current_comm <- do.call(masterEqMetacomm_test, epoch_args)

    # Save the result
    trajectory[[i]] <- current_comm[[1]]
    additional_output[[i]] <- corrent_comm[[2]]
  ...
  return( list(trajectory, additional_output) )
}
